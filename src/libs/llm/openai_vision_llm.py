...
class OpenAIVisionLLM(BaseVisionLLM):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        vision_provider = (
            getattr(vision_settings, "provider", None) if vision_settings else None
        ) or getattr(settings.llm, "provider", None)
        yaml_key = (
            getattr(vision_settings, "api_key", None) if vision_settings else None
        )
        if not (yaml_key and str(yaml_key).strip()):
            yaml_key = getattr(settings.llm, "api_key", None)
