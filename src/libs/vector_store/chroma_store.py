...
class ChromaStore(BaseVectorStore):
    ...
    def __init__(self, settings: Settings, **kwargs: Any) -> None:
        ...
        # If internal metadata files are partially deleted/corrupted (e.g. sqlite
        # schema missing "tenants"), auto-heal by resetting only Chroma data files.
    ...
    def _create_client_with_recovery(self) -> Any:
        """Create a persistent ChromaDB client with one-shot recovery.

        Returns:
            ChromaDB PersistentClient instance.

        Raises:
            RuntimeError: If client initialization fails and cannot be recovered.
        """
        try:
            ...
            # Force a lightweight metadata query so schema issues fail fast here.
            client.list_collections()
            return client
        except Exception as first_error:
            if self._is_recoverable_schema_error(first_error):
                logger.warning(
                    "Detected corrupted Chroma metadata at '%s' (%s). "
                    "Resetting Chroma data files and retrying initialization once.",
                    self.persist_directory,
                    first_error,
                )
                self._reset_chroma_data_files()
                try:
                    client = chromadb.PersistentClient(
                        path=str(self.persist_directory),
                        settings=ChromaSettings(
                    ...
                    client.list_collections()
                    logger.info("Chroma metadata recovery succeeded.")
                    return client
                except Exception as retry_error:
                    raise RuntimeError(
                        f"Failed to recover and initialize ChromaDB client at "
                        f"'{self.persist_directory}': {retry_error}"
                    ) from retry_error

            raise RuntimeError(
                f"Failed to initialize ChromaDB client at '{self.persist_directory}': {first_error}"
            ) from first_error

    @staticmethod
    def _is_recoverable_schema_error(error: Exception) -> bool:
        """Return True when the error matches known recoverable schema failures."""
        message = str(error).lower()
        return (
            "no such table: tenants" in message
            or "database error: no such table" in message
        )

    def _reset_chroma_data_files(self) -> None:
        """Delete Chroma files under persist_directory while keeping folder."""
        self.persist_directory.mkdir(parents=True, exist_ok=True)
        for entry in self.persist_directory.iterdir():
            try:
                if entry.is_dir():
                    shutil.rmtree(entry)
                else:
                ...
                raise RuntimeError(
                    f"Failed to clear Chroma data entry '{entry}': {e}"
                ) from e
