...
class SQLiteIntegrityChecker(FileIntegrityChecker):
    ...
    def list_processed(
        ...
        try:
            ...
            if collection is not None:
                # Treat NULL collection as default namespace (legacy / CLI paths).
                query += " AND IFNULL(collection, 'default') = ?"
    ...
    def list_history(
        ...
        *,
        ...
        """List ingestion rows (success and failed) for dashboard history.

        Args:
            collection: When set, filter with ``IFNULL(collection,'default')``.
            statuses: Subset of ``success``, ``failed``. Default: both.
            limit: Max rows, newest ``updated_at`` first.

        Returns:
            Dicts with file_hash, file_path, status, collection, error_msg,
            processed_at, updated_at.
        """
        want = statuses or ["success", "failed"]
