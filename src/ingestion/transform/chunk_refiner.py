...
class ChunkRefiner(BaseTransform):
    ...
    def _transform_parallel(
        ...
        logger.info(f"Refining {len(chunks)} chunks in parallel (workers={max_workers})...")
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for future in as_completed(future_to_idx):
                ...
                if completed % 30 == 0 or completed == len(chunks):
                    logger.info(f"  Refiner progress: {completed}/{len(chunks)} chunks")
    ...
    def _rule_based_refine(self, text: str) -> str:
        ...
        # 4. Remove obvious extraction garbage lines (symbols/digits only)
        text = re.sub(
            r"(?m)^[\s!\"#$%&'()*+,\-./:;<=>?@[\\\]^_`{|}~\d]{8,}$",
            "",
            text,
        ...
        # 5. Rebuild paragraph boundaries for merged numbered items
        text = re.sub(
            r"(?<=[。！？!?；;:：])(?=(?:\(?\d+\)|\d+[\.、]))",
            "\n",
            text,
        )
        # If extractor returns a single huge line, split after sentence punctuation.
    ...
    @staticmethod
    def _extract_image_placeholders(text: str) -> set[str]:
        """Extract normalized image placeholder IDs from text."""
        if not text:
            return set()
        return {m.strip() for m in IMAGE_PLACEHOLDER_PATTERN.findall(text)}

    def _guard_llm_image_placeholders(
        self,
        original_text: str,
        llm_text: str,
        chunk_id: str,
    ) -> Optional[str]:
        """Prevent LLM from introducing synthetic image placeholders.

        If LLM output introduces new ``[IMAGE: ...]`` IDs that don't exist in the
        original chunk, strip them from the text instead of rejecting.
        """
        original_ids = self._extract_image_placeholders(original_text)
        llm_ids = self._extract_image_placeholders(llm_text)

        if not llm_ids:
            return llm_text

        new_ids = llm_ids - original_ids
        if new_ids:
            logger.warning(
                "ChunkRefiner stripped LLM-generated image placeholders for chunk %s: %s",
                chunk_id,
                sorted(new_ids),
            )
            cleaned = IMAGE_PLACEHOLDER_PATTERN.sub('', llm_text)
            cleaned = re.sub(r'\n{3,}', '\n\n', cleaned)
