...
class MetadataEnricher(BaseTransform):
    ...
    def _transform_parallel(
        ...
        logger.info(f"Enriching {len(chunks)} chunks in parallel (workers={max_workers})...")
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for future in as_completed(future_to_idx):
                ...
                completed += 1
                if completed % 30 == 0 or completed == len(chunks):
                    logger.info(f"  Enricher progress: {completed}/{len(chunks)} chunks")
    ...
    # Pattern for Chinese textbook chapter/section headers to truncate inline body text
    _CN_CHAPTER_RE = re.compile(r'^(#{0,6}\s*第[一二三四五六七八九十百]+章\s*\S+)')
    _CN_SECTION_RE = re.compile(r'(\d+(?:\.\d+){1,2})\s*\S')
    _TITLE_BODY_BOUNDARY_RE = re.compile(r'(?:\d+\.\d+(?:\.\d+)?\s*\S)|[。；;]')
    ...
    def _extract_title(self, text: str) -> str:
        ...
        Priority:
            ...
            2. Chinese textbook chapter header (第X章 ...)
            3. Chinese textbook section header (N.N ...)
        ...
        # 2. Chinese textbook: chapter header like "第七章 相交线与平行线..."
        ch_match = self._CN_CHAPTER_RE.match(first_line)
        if ch_match:
            raw = ch_match.group(1)
            # Truncate at first body-text boundary (section number, period, or semicolon)
            boundary = self._TITLE_BODY_BOUNDARY_RE.search(raw)
            if boundary:
                raw = raw[:boundary.start()].strip()
            # Remove trailing punctuation if it's a chapter separator
            raw = raw.rstrip('，,。；;：:、')
            if raw:
                return raw

        # 3. Chinese textbook: section header like "7.1 相交线" or "7.1.1 两条直线相交"
        sec_match = self._CN_SECTION_RE.match(first_line)
        if sec_match:
            raw = first_line
            boundary = self._TITLE_BODY_BOUNDARY_RE.search(raw, sec_match.end())
            if boundary:
                # Keep the section number + title, strip body text after
                end = boundary.start()
                raw = raw[:end].strip()
            raw = raw.rstrip('，,。；;：:、')
            if len(raw) <= 80:
        ...
        # 4. Use first line if it's short and looks like a title
        if first_line and len(first_line) <= 100 and not first_line.endswith(('.', ',', ';')):
            return first_line

        # 5. Use first sentence (without trailing punctuation)
    ...
    def _extract_tags(self, text: str, max_tags: int = 10) -> List[str]:
        """Extract tags using jieba keyword extraction for Chinese + regex for English.

        Uses jieba.analyse.extract_tags with TF-IDF weighting, which works well
        for both Chinese and mixed-language text. Falls back to frequency-based
        extraction if jieba.analyse is unavailable.
        """
        if not text:
            return []
        ...
        try:
            ...
            # jieba TF-IDF extraction: returns (term, weight) pairs
            keywords = jieba.analyse.extract_tags(
                text, topK=max_tags, withWeight=False,
            )
            # Filter out single-character terms and pure digits
            keywords = [k for k in keywords if len(k) > 1 and not k.isdigit()]
            return keywords[:max_tags]
        except Exception:
            pass

        # Fallback: frequency-based extraction via jieba.cut
        try:
            import jieba
            from collections import Counter
            words = [w for w in jieba.cut(text) if len(w) > 1 and not w.isdigit()]
            counter = Counter(words)
            # Exclude very common words (appearing in >80% of the text is probably not a tag)
            return [w for w, _ in counter.most_common(max_tags * 2)][:max_tags]
        except Exception:
            return []
    ...
    def _parse_llm_response(self, response: str) -> Dict[str, Any]:
        ...
        chapter_match = re.search(r'Chapter_num:\s*(\d+(?:\.\d+)?)', response, re.IGNORECASE)
        if chapter_match:
            try:
                metadata['chapter_num'] = int(float(chapter_match.group(1)))
            except ValueError:
                metadata['chapter_num'] = 0

        section_match = re.search(r'Section_num:\s*(\d+(?:\.\d+)?)', response, re.IGNORECASE)
        if section_match:
            try:
                metadata['section_num'] = float(section_match.group(1))
            except ValueError:
                metadata['section_num'] = 0

        summary_match = re.search(
            r'Summary:\s*(.+?)(?:\n(?:Chapter_num|Section_num|Tags|Content_type):|$)',
        ...
        ct_match = re.search(r'Content_type:\s*(\S+)', response, re.IGNORECASE)
        if ct_match:
            metadata['content_type'] = ct_match.group(1).strip().lower()
