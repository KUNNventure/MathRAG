"""Ingestion Pipeline orchestrator for the MathRAG MCP Server.
...
# ImageCaptioner disabled — extract_images=False, no vision LLM credits
...
# Match image placeholders in chunk text (same pattern family as ChunkRefiner)
_IMAGE_PLACEHOLDER_RE = re.compile(r"\[IMAGE:\s*([^\]]+)\]")


def _chunk_dict_for_transform_trace(chunk: Chunk, text_before: str) -> Dict[str, Any]:
    """Build one transform-stage trace row with full chunk metadata for the dashboard."""
    meta = chunk.metadata or {}
    text_after = chunk.text or ""
    caps = meta.get("image_captions")
    cap_keys: List[str] = []
    if isinstance(caps, dict):
        cap_keys = [str(k) for k in caps.keys()]
    failures = meta.get("image_caption_failures")
    fail_n = 0
    if isinstance(failures, list):
        fail_n = len(failures)
    elif isinstance(failures, dict):
        fail_n = len(failures)
    stripped = text_after.strip()
    kept_for_embedding = bool(stripped) and len(stripped) >= 10
    return {
        "chunk_id": chunk.id,
        ...
        "char_len": len(text_after),
        "refined_by": meta.get("refined_by", ""),
        "enriched_by": meta.get("enriched_by", ""),
        "refine_fallback_reason": meta.get("refine_fallback_reason"),
        "enrich_fallback_reason": meta.get("enrich_fallback_reason"),
        "enrich_error": meta.get("enrich_error"),
        ...
        "image_placeholder_count": len(_IMAGE_PLACEHOLDER_RE.findall(text_after)),
        "image_caption_count": len(cap_keys),
        "image_caption_image_ids": cap_keys[:24],
        "image_caption_failure_count": fail_n,
        "kept_for_embedding": kept_for_embedding,
    }
...
class IngestionPipeline:
    ...
    def __init__(
        ...
        Args:
            ...
            grade: Optional grade level metadata for textbook chunking
            volume: Optional volume metadata for textbook chunking
            document_metadata: Extra fields merged into ``Document.metadata`` (and
                inherited by chunk metadata), e.g. ``grade``, ``volume``, ``collection``.
                Non-string values are copied as-is; empty strings are skipped.
                ``grade`` / ``volume`` constructor kwargs override the same keys here.
        """
        ...
        self.grade = grade
        self.volume = volume
        self._document_metadata = dict(document_metadata) if document_metadata else {}
    ...
    def run(
        ...
        try:
            ...
            if not self.force and self.integrity_checker.should_skip(file_hash):
                ...
                _integrity_elapsed = (time.monotonic() - _integrity_t0) * 1000.0
            ...
            # ─────────────────────────────────────────────────────────────
            # Stage 2: Document Loading (or read from cache)
            # ─────────────────────────────────────────────────────────────
            ...
            # Merge ingestion metadata into document (inherited by all chunks)
            merged: Dict[str, Any] = dict(self._document_metadata)
            if self.grade is not None and str(self.grade).strip() != "":
                merged["grade"] = str(self.grade).strip()
            if self.volume is not None and str(self.volume).strip() != "":
                merged["volume"] = str(self.volume).strip()
            merged["collection"] = self.collection
            for key, val in merged.items():
                if val is None:
                    continue
                if isinstance(val, str) and val.strip() == "":
                    continue
                document.metadata[key] = val.strip() if isinstance(val, str) else val
            ...
            # ─────────────────────────────────────────────────────────────
            # Stage 4: Transform Pipeline (chunk refiner + metadata enricher)
            # ─────────────────────────────────────────────────────────────
            refine_mode = "llm" if self.chunk_refiner.use_llm else "rule"
            enrich_mode = "llm" if self.metadata_enricher.use_llm else "rule"
            logger.info(
                "\n🔄 Stage 4: Transform Pipeline (refiner=%s, enricher=%s)",
                refine_mode,
                enrich_mode,
            )
            _notify("transform", 4)
            ...
            _pre_drop = len(chunks)
            transform_chunk_rows: List[Dict[str, Any]] = []
            captioned_chunks = 0
            for c in chunks:
                row = _chunk_dict_for_transform_trace(c, _pre_refine_texts.get(c.id, ""))
                transform_chunk_rows.append(row)
                ta = row.get("text_after") or ""
                if "Description:" in ta or int(row.get("image_caption_count") or 0) > 0:
                    captioned_chunks += 1

            dropped_before_embed = sum(
                1 for row in transform_chunk_rows if not row.get("kept_for_embedding")
            )
            kept_for_embed = _pre_drop - dropped_before_embed

            _transform_method = f"chunk_refiner({refine_mode})+metadata_enricher({enrich_mode})"
            stages["transform"] = {
                "chunk_refiner": {
                    "llm": refined_by_llm,
                    "rule": refined_by_rule,
                    "error": refined_by_error,
                },
                "metadata_enricher": {
                    "llm": enriched_by_llm,
                    "rule": enriched_by_rule,
                    "error": enriched_by_error,
                },
                "image_captioner": {
                    "enabled": False,
                    "captioned_chunks": captioned_chunks,
                },
                "chunks_after_transform": _pre_drop,
                "chunks_dropped_before_embed": dropped_before_embed,
                "chunks_kept_for_embed": kept_for_embed,
            }
            ...
            if trace is not None:
                trace.record_stage(
                    "transform",
                    {
                        ...
                        "chunk_refiner_use_llm": bool(self.chunk_refiner.use_llm),
                        "metadata_enricher_use_llm": bool(self.metadata_enricher.use_llm),
                        "image_caption_enabled": False,
                        "refined_by_llm": refined_by_llm,
                        "refined_by_rule": refined_by_rule,
                        "refined_by_error": refined_by_error,
                        "enriched_by_llm": enriched_by_llm,
                        "enriched_by_rule": enriched_by_rule,
                        "enriched_by_error": enriched_by_error,
                        "captioned_chunks": captioned_chunks,
                        "chunks_after_transform": _pre_drop,
                        "chunks_dropped_before_embed": dropped_before_embed,
                        "chunks_kept_for_embed": kept_for_embed,
                        "chunks": transform_chunk_rows,
                    },
                    elapsed_ms=_elapsed_transform,
                )

            # Drop empty/near-empty chunks before encoding (refiner may clear some)
            chunks = [
                c for c in chunks
                if c.text and len(c.text.strip()) >= 10
            ]
            if len(chunks) < _pre_drop:
                logger.info(
                    "  Dropped %s empty/near-empty chunks before encoding",
                    _pre_drop - len(chunks),
                )
                for i, c in enumerate(chunks):
                    if c.metadata:
            ...
            # Record splitter settings on each chunk for later audits (overlap/size tier).
            ing_cfg = self.settings.ingestion
            if ing_cfg is not None:
                _isz = int(ing_cfg.chunk_size)
                _iov = int(ing_cfg.chunk_overlap)
                _isp = str(ing_cfg.splitter)
                for c in chunks:
                    meta = c.metadata
                    if not isinstance(meta, dict):
                        continue
                    meta["ingestion_chunk_size"] = _isz
                    meta["ingestion_chunk_overlap"] = _iov
                    meta["ingestion_splitter"] = _isp
