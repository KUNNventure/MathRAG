...
class RagasEvaluator(BaseEvaluator):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        Args:
            ...
            judge_model: Override LLM model for RAGAS judging.
                Defaults to settings.llm.model. Use a cheaper model
                (e.g. "qwen-plus") to reduce cost without losing
                relative-ranking fidelity.
        ...
        self._wrappers_cache: Optional[tuple] = None
    ...
    def evaluate(
        ...
        **kwargs: Any,
        ...
        Args:
            ...
            ground_truth: Optional; ``reference_answer`` used for Context Recall.
        ...
        # Extract reference text from ground_truth (for Context Recall)
        reference = self._extract_reference(ground_truth)

        try:
            result = self._run_ragas(query, contexts, generated_answer, reference=reference)
    ...
    def _run_ragas(
        ...
        - ContextRecall: (user_input, response, retrieved_contexts, reference)
        ...
        # Build LLM / Embedding wrappers from settings (cached)
        if self._wrappers_cache is None:
            self._wrappers_cache = self._build_wrappers()
        llm, embeddings = self._wrappers_cache
        ...
        for metric_name in self._metric_names:
            if metric_name == FAITHFULNESS:
                ...
                ref_text = reference if reference else answer
                result = m.score(
                    user_input=query,
                    retrieved_contexts=contexts, reference=ref_text,
                )
            else:
                continue
    ...
    def _build_wrappers(self) -> tuple:
        ...
        openai_compatible_llm_providers = {"openai", "qwen", "deepseek", "gemini"}

        from src.core.api_keys import require_api_key

        llm_api_key = require_api_key(
            llm_cfg.api_key, provider=provider, service="Ragas LLM"
        )
        ...
        if use_azure_llm:
            ...
            raise ValueError(
                ...
                "Supported: azure, openai, qwen, deepseek, gemini"
            )

        judge_model = self._judge_model or llm_cfg.model
        llm = llm_factory(judge_model, client=llm_client, max_tokens=8192)
        ...
        openai_compatible_embedding_providers = {"openai", "qwen", "gemini", "deepseek"}

        emb_api_key = require_api_key(
            emb_cfg.api_key, provider=emb_provider, service="Ragas embedding"
        )
    ...
    @staticmethod
    def _extract_reference(ground_truth: Any) -> Optional[str]:
        """Extract reference answer text from ground_truth."""
        if ground_truth is None:
            return None
        if isinstance(ground_truth, str):
            return ground_truth
        if isinstance(ground_truth, dict):
            return ground_truth.get("reference_answer") or ground_truth.get("answer")
