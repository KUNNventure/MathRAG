"""Configuration loading and validation for the MathRAG MCP Server."""
...
def resolve_path(relative: Union[str, Path]) -> Path:
    ...
    PosixPath('/home/user/MathRAG/config/settings.yaml')
    """
    p = Path(relative)
    if p.is_absolute():
        return p
...
class RetrievalSettings:
    ...
    fusion_top_k: int
    rrf_k: int
    # Per-list weights for RRF fusion (dense, sparse). Default 1.0 = standard RRF.
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "Settings":
        ...
        if "generation" in data:
            ...
            generation_settings = GenerationSettings(
                prompt_path=_require_str(generation, "prompt_path", "generation"),
            )
        ...
        settings = cls(
            ...
            retrieval=RetrievalSettings(
                ...
                dense_rrf_weight=float(retrieval.get("dense_rrf_weight", 1.0)),
                sparse_rrf_weight=float(retrieval.get("sparse_rrf_weight", 1.0)),
            ),
            rerank=RerankSettings(
...
def load_settings(path: str | Path | None = None) -> Settings:
    ...
    """
    from src.core.api_keys import load_dotenv_if_present
