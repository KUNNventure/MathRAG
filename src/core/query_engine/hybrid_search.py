...
class HybridSearch:
    ...
    def _extract_config(self, settings: Optional[Settings]) -> HybridSearchConfig:
        ...
        return HybridSearchConfig(
            ...
            dense_rrf_weight=float(getattr(retrieval_config, 'dense_rrf_weight', 1.0)),
            sparse_rrf_weight=float(getattr(retrieval_config, 'sparse_rrf_weight', 1.0)),
        )
    ...
    def _fuse_results(
        ...
        # Build ranking lists for RRF (weights align with [dense, sparse] order)
        ranking_lists: List[List[RetrievalResult]] = []
        weights: List[float] = []
        if dense_results:
            ranking_lists.append(dense_results)
            weights.append(self.config.dense_rrf_weight)
        if sparse_results:
            ranking_lists.append(sparse_results)
            weights.append(self.config.sparse_rrf_weight)
...
def create_hybrid_search(
    ...
    Args:
        ...
        config: Optional HybridSearchConfig (e.g. dense-only / RRF weights). If None, derived from settings.
